...
import { Product, Branch, InventoryItem, Waste } from '@/types';
...
export default function EditWastePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setOriginalQuantity(wasteData.quantity);
        
        // Set form values
        reset({
          reference_number: wasteData.reference_number,
        ...
        setSelectedProductCost(wasteData.cost / wasteData.quantity);
        setTotalCost(wasteData.cost);
      } catch (err: any) {
  ...
  // تحديث التكلفة الإجمالية عند تغيير الكمية
  useEffect(() => {
  ...
  const getAvailableInventory = () => {
    ...
    // إضافة الكمية الأصلية للهدر إلى المخزون المتاح (لأنها ستعود للمخزون عند التعديل)
    const availableQuantity = inventoryItem ? inventoryItem.quantity : 0;
    return availableQuantity + (watchProductId === waste?.product_id && watchBranchId === waste?.branch_id ? originalQuantity : 0);
  };
  ...
  const onSubmit = async (data: WasteFormValues) => {
    ...
    try {
      ...
      setError(err.message || 'حدث خطأ أثناء تحديث سجل الهدر');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <RoleGuard allowedRoles={['admin', 'branch_manager']}>
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">تعديل سجل الهدر</h1>
        </div>
