...
import { Product, Branch, PurchaseInvoice, PurchaseItem } from '@/types';
...
export default function EditPurchasePage() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        reset({
          ...
          items: itemsData.map((item: PurchaseItem) => ({
            id: item.id,
        ...
        setTotalVAT(invoiceData.vat_amount);
      } catch (err: any) {
  ...
  const onSubmit = async (data: PurchaseInvoiceFormValues) => {
    ...
    try {
      ...
      setError(err.message || 'حدث خطأ أثناء تحديث فاتورة الشراء');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <RoleGuard allowedRoles={['admin', 'branch_manager']}>
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">تعديل فاتورة الشراء</h1>
        </div>
